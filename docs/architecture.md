...
`worker/providers/llm/provider_registry.py` declares every provider *adapter* that exists in code (`PROVIDER_SLOTS`) — Gemini, Groq 8B, Groq 70B, Mistral, Together, Cohere, Cerebras, and 4 OpenRouter free models. Every one of those is a fixed model string except **Cerebras**, whose free-tier catalog has changed shape before: `build_enabled_slots()` calls Cerebras's live `/v1/models` endpoint on each waterfall build and turns whatever it finds into slots (`_discover_cerebras_slots()`), so a model Cerebras adds later shows up as a new fallback automatically, no deploy needed. If that call fails for any reason (network, bad key, endpoint change) it falls back to `_CEREBRAS_FALLBACK_MODELS`, a small hardcoded list (`gpt-oss-120b`, `gemma-3-31b`, `zai-glm-4.7` as of when this was written) — so a Cerebras outage can't break every other provider's slot-building. The dashboard's `ask_ai`/`recommendations` side (`lib/llm-waterfall.ts`) stays on a single fixed Cerebras model rather than discovering live, since that runs per-request (would add latency/cost to every question) rather than once per pipeline run. `build_enabled_slots(config)` resolves the full slot list against:

1. Whether the slot's env var (e.g. `OPENROUTER_API_KEY`) is actually set
2. Admin-configured `enabled`/`priority` overrides in `llm_provider_config` (scope `pipeline`), editable at `/admin/llm-providers` without a deploy — note that auto-discovered Cerebras models only have a toggle row here if `/admin/llm-providers`' static mirror list happens to include that model's key; new models still work in the actual waterfall either way, just without an individual toggle until the mirror list is updated
