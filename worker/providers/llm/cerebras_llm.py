...
class CerebrasLLMProvider(LLMProvider):
    ...
    def extract_insights(self, episode: Episode, transcript: Transcript, domain: str) -> Insight:
        if len(transcript.text) > _MAX_TRANSCRIPT_CHARS:
            data = chunked_extract(
                ...
                transcript.text, _CHUNK_TARGET_CHARS, log_prefix="    [Cerebras]",
            )
        else:
    ...
    def _generate_text(self, prompt: str) -> str:
        ...
        for attempt, delay in enumerate([0] + _RETRY_DELAYS):
            if delay:
                print(f"    [Cerebras] retry {attempt}/{len(_RETRY_DELAYS)} in {delay}s…")
                time.sleep(delay)
            try:
                ...
                if not response.ok:
                    if response.status_code == 429 or "quota" in body.lower() or "rate" in body.lower():
                        last_exc = RuntimeError(f"Cerebras {response.status_code}: {body[:300]}")
                        continue
                    raise RuntimeError(f"Cerebras {response.status_code}: {body[:300]}")
                data = response.json()
