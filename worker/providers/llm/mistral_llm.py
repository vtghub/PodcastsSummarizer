"""Mistral LLM provider — La Plateforme free tier, via plain REST (no SDK)."""
...
from worker.config.settings import MISTRAL_API_KEY, MISTRAL_MODEL
...
# Conservative free-trial budget — Mistral's exact free-tier rate limits
# aren't published with the same clarity as Groq's, so this mirrors Groq's
# conservative sizing rather than assuming a larger safe request.
...
class MistralLLMProvider(LLMProvider):
    ...
    def extract_insights(self, episode: Episode, transcript: Transcript, domain: str) -> Insight:
        if len(transcript.text) > _MAX_TRANSCRIPT_CHARS:
            data = chunked_extract(
                ...
                transcript.text, _CHUNK_TARGET_CHARS, log_prefix="    [Mistral]",
            )
        else:
    ...
    def _generate_text(self, prompt: str) -> str:
        """Call Mistral with retry-on-transient-error, returning raw response text."""
        last_exc: Exception | None = None
        for attempt, delay in enumerate([0] + _RETRY_DELAYS):
            if delay:
                print(f"    [Mistral] retry {attempt}/{len(_RETRY_DELAYS)} in {delay}s…")
                time.sleep(delay)
            try:
                ...
                if not response.ok:
                    if response.status_code == 429 or "quota" in body.lower() or "rate" in body.lower():
                        last_exc = RuntimeError(f"Mistral {response.status_code}: {body[:300]}")
                        continue
                    raise RuntimeError(f"Mistral {response.status_code}: {body[:300]}")
                data = response.json()
