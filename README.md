...
│       ├── 023_weekly_recommendations_toggle.sql # weekly_recommendations_enabled BOOLEAN (default TRUE) on user_profiles — admin-controlled, independent of digest_enabled
│       └── 024_episode_notes.sql    # episode_notes table (keyed on episode_id, not insight_id) with RLS (per-user SELECT/INSERT/UPDATE/DELETE) — private multi-note-per-episode annotations
│
...
│   │   ├── login/page.tsx           # Email + password sign-in; "Forgot password?" link to /forgot-password
│   │   ├── forgot-password/page.tsx # Password reset request — email entry, calls supabase.auth.resetPasswordForEmail() client-side
│   │   ├── reset-password/page.tsx  # Password reset landing page — detects the recovery session from the email link, sets a new password via supabase.auth.updateUser()
...
│   │       ├── episodes/[id]/notes/ # GET list caller's notes for this episode · POST add a note (authed; 401 otherwise)
│   │       ├── episode-notes/[id]/  # PATCH edit note body (debounced autosave) · DELETE — both ownership-checked (authed)
...
│   │   ├── InsightCard.tsx          # Per-episode insight with read-aloud, bookmark toggle (☆/★), engagement bar (incl. a Sparkles "Ask AI about this episode" button deep-linking to /ask?episode=<id>, and a StickyNote "My notes" toggle opening NotesPanel); shows episode.title_en (English translation) in place of a non-English title when available
...
│   ├── retry_failed_episodes.yml    # Cron 4×/day (3, 9, 15, 21 UTC), offset from daily_pipeline — dedicated recovery pass for episodes that failed ingestion, with a fresh waterfall instance each run; optional limit input
...
auth.users  (Supabase Auth)
    │
    ├── user_profiles       display_name, is_admin, digest_enabled, weekly_recommendations_enabled, digest_hour, digest_domains[], digest_frequency, digest_day_of_week, last_visited_at
    │
...
episodes (keyed independently of insights — an episode can exist with zero insight rows)
    │
    └── episode_notes  private, multi-note-per-episode annotations per user (editable, deletable)
```

- **Guests**: see all public insights (unfiltered preview); views are tracked anonymously
- **Signed-in users**: see only insights from their subscribed sources; can like, dislike, comment, and keep private notes on any episode
- **Admins** (`is_admin=TRUE`): full catalog management (add/enable/disable/delete sources); `/admin/users` page for listing users, granting/revoking admin, resetting onboarding, subscribing/unsubscribing any user to/from any podcast, enabling/disabling any user's email digest (`digest_enabled`, daily or weekly per their own frequency setting), subscribing/unsubscribing any user to/from the Weekly Recommendations email (independent of their digest setting), and cascade-deleting a user (`auth.admin.deleteUser` removes `auth.users` and every `ON DELETE CASCADE` row across `user_profiles`, `user_subscriptions`, `insight_bookmarks`, `insight_reactions`, `insight_comments`, `comment_reactions`, `episode_notes` in one call — the safe way to fully remove a user, vs. manually deleting rows from individual tables)
- **RLS**: `user_profiles`, `user_subscriptions`, `sources`, `insight_reactions`, `insight_bookmarks`, `insight_comments`, `comment_reactions`, `episode_notes` all have row-level security; `insights` and `insight_views` are public-readable; `insight_views` additionally allows authenticated users to delete their own rows (Mark as Unread); migration 016 adds `is_admin_user()` (SECURITY DEFINER) and a `profiles_admin_select_all` policy so admins can read every `user_profiles` row (previously self-only) — required for the Manage Users page and its live Realtime updates; migration 016 also adds `user_profiles` to the `supabase_realtime` publication; `episode_notes` (migration 024) is the only engagement table with an UPDATE policy alongside SELECT/INSERT/DELETE, since notes are edited in place
...
| **Episode Notes** | Signed-in users can keep multiple private notes per episode via the teal StickyNote toggle on the engagement bar, opening a lazily-loaded panel (`NotesPanel`). Fully auto-save, no Save button: typing in "Add a note" creates a new note ~600ms after the user stops typing; editing an existing note (`NoteRow`) autosaves the same way with a small Saving…/Saved indicator; delete uses a confirm-then-delete inline control, same pattern as Comments. Keyed on `episode_id` (not `insight_id` like other engagement tables), so notes work even for episodes with no insight row yet. `/notes` page (mirrors `/saved`) lists every episode with notes, grouped and collapsible, most-recently-active first; **My Notes** link in the navbar "More" dropdown (signed-in users only) |
...
| **Auth** | Supabase email + password; SSR JWT cookies; RLS enforced at DB level; "Forgot password?" on `/login` → `/forgot-password` (email entry, `resetPasswordForEmail()`) → emailed link → `/reset-password` (detects the recovery session from the URL, sets a new password via `updateUser()`) — all client-side against Supabase Auth, no new API routes |
...
| **Task Status (admin)** | Admin-only `/admin/task-status` page with three sections: **GitHub Actions Runners** — every workflow in the repo with its most recent run status, a "Run now" button (workflow_dispatch), and a "Cancel" button when a run is queued/in progress, polled every 20s; **Failed Episodes** — episodes stuck in `episode_queue` with `status='failed'` (title, source, retry count, last error, last updated), with a "Retry now" button that dispatches `retry_failed_episodes.yml` immediately instead of waiting for its next scheduled run; and **Episode Transcription Detail** — the 15 most recently chunked episodes, expandable to show every chunk's LLM model, status, timestamp, and error message, with three outcome states per episode (not just success/fail): a clean green check for succeeded-first-try, an amber "Recovered after retry" badge for episodes where a chunk failed but a later retry got it through (`episodes.status='done'` despite `extraction_chunk_log` showing a failure somewhere in its history), and a red X only for episodes still genuinely stuck. |
