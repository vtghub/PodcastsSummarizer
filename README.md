...
- **Admins** (`is_admin=TRUE`): full catalog management (add/enable/disable/delete sources); `/admin/users` page for listing users, granting/revoking admin, resetting onboarding, subscribing/unsubscribing any user to/from any podcast, enabling/disabling any user's email digest (`digest_enabled`, daily or weekly per their own frequency setting), subscribing/unsubscribing any user to/from the Weekly Recommendations email (independent of their digest setting), and cascade-deleting a user (`auth.admin.deleteUser` removes `auth.users` and every `ON DELETE CASCADE` row across `user_profiles`, `user_subscriptions`, `insight_bookmarks`, `insight_reactions`, `insight_comments`, `comment_reactions` in one call — the safe way to fully remove a user, vs. manually deleting rows from individual tables)
