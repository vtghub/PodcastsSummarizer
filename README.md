...
| **Export** | Signed-in users click "↓ Export ▾" next to the date navigator to open a dropdown with three formats — **CSV** (download spreadsheet with columns: Date, Domain, Source, Episode, Summary, Key Points, Key Quotes, Action Items, Tags; pipe-separated within cells), **JSON** (pretty-printed download with all fields as arrays), **PDF** (opens printable HTML in a new tab — use browser print dialog to save as PDF); all formats served by `GET /api/insights/export?format=csv|json|pdf&date=YYYY-MM-DD` (auth required) |
