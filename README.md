...
| **LLM Providers (admin)** | Admin-only `/admin/llm-providers` page — three independently toggle/reorder-able sections, each editing its own `llm_provider_config` scope: **Pipeline Extraction** (worker's insight-extraction waterfall — Gemini, Groq 8B/70B, Mistral, Cohere, Cerebras, 4× OpenRouter free models) and **Ask AI** / **Recommendations** (the dashboard's own `lib/llm-waterfall.ts` — same 11-provider set: Gemini, Groq 8B/70B, Mistral, Together, Cohere, Cerebras, 4× OpenRouter, since both scopes call the exact same `runWaterfall()` function). The pipeline picks up changes on its next run; Ask AI/Recommendations pick them up on the next question/refresh. Each row shows whether its API key is detected in the current environment. |
